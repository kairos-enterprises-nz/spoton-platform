 ...
 */
export function getStepCompletionStatus(normalizedData) {
  const stepKeys = ['aboutYou', 'yourServices', 'serviceDetails', 'howYoullPay', 'preferences', 'confirmation'];
  const completionStatus = stepKeys.map(stepKey => {
    const isComplete = isStepComplete(normalizedData, stepKey);
 ...
 */
export function isStepComplete(normalizedData, stepKey) {
  ...
  switch (stepKey) {
    ...
    case 'yourServices': {
      ...
      // Validate plans based on selected services
      if (hasElectricity && (!selectedPlans.electricity || !selectedPlans.electricity.pricing_id)) {
        return false;
      }
      if (hasBroadband && (!selectedPlans.broadband || !selectedPlans.broadband.pricing_id)) {
        return false;
      }
      if (hasMobile && (!selectedPlans.mobile || (!selectedPlans.mobile.pricing_id && !selectedPlans.mobile.plan_id))) {
        return false;
      }

      // Check if address is selected - but ONLY for electricity/broadband services
      // Mobile services don't require an address
      if ((hasElectricity || hasBroadband)) {
        const selectedAddress = stepData.selectedAddress || 
                               stepData.addressInfo || 
                               normalizedData.selectedAddress || 
                               normalizedData.initialSelection?.address;
        if (!selectedAddress || !selectedAddress.full_address) return false;
      }
