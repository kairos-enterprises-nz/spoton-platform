    ...
    <div className="space-y-3">
      ...
      >
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4">
          <div className="flex items-center space-x-3 mb-3 lg:mb-0">
            <div className="flex items-center space-x-2 px-4 py-2 rounded-lg bg-gradient-to-r from-accent-red to-accent-red/80 text-white shadow-sm">
              <CreditCard className="h-4 w-4" />
              <span className="font-bold text-lg">SpotOn Billing</span>
            </div>
            <div>
              <h1 className="text-xl lg:text-2xl font-bold text-secondary-darkgray">
                Billing & Payments
              </h1>
              <p className="text-sm text-gray-600">
                Manage bills & account balance
              </p>
            </div>
          </div>
      ...
      >
        ...
        <ServiceCard
          title="Next Bill"
          icon={Receipt}
          value={`$${billingData.nextBillAmount.toFixed(2)}`}
          subtitle={`Due ${new Date(billingData.nextBillDate).toLocaleDateString()}`}
          color="accent-red"
        />
        ...
        <ServiceCard
          ...
          icon={DollarSign}
          value={`$${billingData.accountCredit.toFixed(2)}`}
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-center">
                  ...
                  <div className="p-4 rounded-lg bg-accent-green/5 border border-accent-green/20">
                    <div className="text-2xl font-bold text-accent-green">
                      ${(maxAmount - currentHistory[0].amount).toFixed(2)}
                    </div>
                    <div className="text-xs text-gray-500">Saved vs Peak</div>
                  </div>
                </div>
              </div>
            </DashboardCard>
          ...
          >
            <DashboardCard>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Payment Method */}
                <div>
                  <h3 className="text-lg font-semibold text-secondary-darkgray flex items-center mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-secondary-darkgray flex items-center mb-4">
                    <TrendingUp className="h-4 w-4 mr-2 text-primary-turquoise" />
                    Spending Trends
                  </h3>
                  ...
                  <div className="space-y-3">
                    <div className="p-3 rounded-lg bg-accent-green/5 border border-accent-green/20">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium text-gray-700">vs Last Month</span>
                        <span className="text-sm font-bold text-accent-green">-8.5%</span>
                      </div>
                      <div className="text-xs text-gray-500 mt-1">You saved $12.30</div>
                    </div>

                    <div className="p-3 rounded-lg bg-primary-turquoise/5 border border-primary-turquoise/20">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium text-gray-700">vs Same Month Last Year</span>
                        <span className="text-sm font-bold text-primary-turquoise">-15.2%</span>
                      </div>
                      <div className="text-xs text-gray-500 mt-1">Annual savings: $245</div>
                    </div>
                    ...
                    <div className="p-3 rounded-lg bg-accent-blue/5 border border-accent-blue/20">
                      ...
                      <div className="text-xs text-gray-500 mt-1">Above average usage efficiency</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <DashboardCard>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-semibold text-secondary-darkgray flex items-center">
                  <DollarSign className="h-4 w-4 mr-2 text-accent-red" />
                  Next Bill
                </h2>
              </div>

              <div className="space-y-4">
                <div className="p-4 rounded-lg bg-accent-red/5 border border-accent-red/20 text-center">
                  <div className="text-3xl font-bold text-accent-red mb-1">${billingData.nextBillAmount.toFixed(2)}</div>
                  <div className="text-sm text-gray-600 mb-2">Due in 5 days</div>
                  <div className="text-xs text-accent-green font-medium">Auto Pay enabled</div>
                </div>

                <div className="grid grid-cols-2 gap-3 text-center">
                  <div className="p-3 rounded bg-gray-50/50">
                    <div className="text-lg font-bold text-secondary-darkgray">${billingData.accountCredit.toFixed(2)}</div>
                    <div className="text-xs text-gray-500">Account Credit</div>
                  </div>
                  <div className="p-3 rounded bg-gray-50/50">
                    <div className="text-lg font-bold text-secondary-darkgray">${billingData.currentBalance.toFixed(2)}</div>
                ...
                <div className="space-y-2">
                  ...
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="w-full p-3 bg-accent-purple/10 text-accent-purple rounded-lg hover:bg-accent-purple/20 transition-all duration-300 font-medium text-sm"
                  >
                    Download Statements
                  </motion.button>
                </div>
            ...
            <DashboardCard>
              ...
              <div className="space-y-4">
                <div className="p-3 rounded-lg bg-accent-green/5 border border-accent-green/20">
                  ...
                  <div className="text-xs text-gray-600">Your account is current with no outstanding balance</div>
                </div>
                ...
                <div className="pt-3 border-t border-gray-200">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-primary-turquoise mb-1">
                      {new Date(billingData.nextBillDate).toLocaleDateString()}
                    </div>
                    <div className="text-xs text-gray-500">Next auto payment</div>
                  </div>
                </div>
              </div>
            </DashboardCard>
