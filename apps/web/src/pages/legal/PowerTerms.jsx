...
import { generalElectricitySections } from "./data/GeneralElectricityTerms";
import { spotPlanTerms } from "./data/SpotPlanTerms";
import { fixedPlanTerms } from "./data/FixedPlanTerms";
import { touPlanTerms } from "./data/TOUPlanTerms";
...
const plans = [
  {
    id: "spot",
    name: "Spot Pricing",
    summary: "Real-time pricing linked to the wholesale market.",
    icon: <Zap className="w-5 h-5 text-primary-turquoise" />,
    content: spotPlanTerms,
    updated: "March 2025",
  },
  {
    id: "fixed",
    name: "Fixed Rate",
    summary: "Stable pricing locked in for the duration of the plan.",
    icon: <FileText className="w-5 h-5 text-primary-turquoise" />,
    content: fixedPlanTerms,
    updated: "April 2025",
  },
  {
    id: "tou",
    name: "Time-of-Use",
    summary: "Different rates for peak vs off-peak usage times.",
    icon: <Clock className="w-5 h-5 text-primary-turquoise" />,
    content: touPlanTerms,
    updated: "April 2025",
  },
];
export default function PowerTermsIndex() {
  ...
  const sections = showGeneral ? generalElectricitySections : selectedPlan?.content || [];
  ...
  useEffect(() => {
    ...
    if (sectionParam === 'general') {
      forceShowGeneral(); // force open when URL says so
    } else if (['spot', 'fixed', 'tou'].includes(planParam)) {
  ...
  return (
    <main className="relative bg-white text-gray-800 scroll-smooth isolate overflow-x-visible">
      <header className="max-w-5xl mx-auto px-6 pt-20 pb-12 text-center relative z-10">
        <h1 className="text-4xl font-extrabold tracking-tight mb-4">Electricity Legals</h1>
        <p className="text-base text-gray-600 max-w-5xl mx-auto">
          Learn about our general electricity terms and conditions or explore the specific legal terms of each available plan.
        </p>
      </header>
      <div className="sticky top-0 z-30 max-w-4xl mx-auto bg-white pt-4">
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          ...
          >
            {showGeneral ? "Hide General Terms" : "View General Electricity Terms"}
          </button>
          <button
            onClick={() => {
        ...
        {(showGeneral || selectedPlan) && (
          <div className="mt-6 pb-2 border-b border-accent-lightturquoise text-center max-w-4xl">
            <h2 className="text-2xl font-bold text-gray-700 mb-3">
              {showGeneral ? "General Electricity Terms" : `${selectedPlan?.name} – Legal Terms`}
            </h2>
            {sections.length > 0 && (
