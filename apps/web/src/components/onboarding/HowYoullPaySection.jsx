...
const HowYoullPaySection = ({ 
  ...
  useEffect(() => {
    if (isFirstRender.current) {
      setSelectedMethod(initialData?.paymentMethodType || null);
      setBankDetails({
        bankName: initialData?.bankName || '',
        accountName: initialData?.accountName || '',
        accountNumber: initialData?.accountNumber || ''
  ...
  }, [selectedMethod, selectedServices, bankDetails, validateForm, onAutosave, initialData?.paymentMethodType, initialData?.bankName, initialData?.accountName, initialData?.accountNumber]);
          ...
          <div className="flex flex-row flex-nowrap gap-4 w-full">
            {activeServices.map(service => {
              ...
              return (
                <div
                  key={service}
                  className={`flex flex-col items-center gap-4 p-4 rounded-xl border-2 ${config.bgColor} border-white/10 bg-gray-800/80 basis-1/3 min-w-0`}
                >
                  <div className={`w-12 h-12 rounded-lg ${config.iconBg} flex items-center justify-center flex-shrink-0`}>
                    <Icon className={`w-6 h-6 ${service === 'electricity' ? 'text-accent-green' : service === 'broadband' ? 'text-accent-purple' : 'text-blue-400'}`} />
                  </div>
                  <div className="flex-1 min-w-0 text-center">
                    <h4 className="font-semibold text-base text-white mb-1 truncate">{config.label}</h4>
                    <div className="flex justify-center items-center gap-2">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium ${config.badgeColor}`}>{config.frequency}</span>
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium ${config.paymentType === 'Prepaid' ? 'bg-amber-500/50 text-white' : 'bg-emerald-500/50 text-white'}`}>{config.paymentType}</span>
                    </div>
                  </div>
                </div>
              );
