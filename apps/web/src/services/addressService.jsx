...
export const searchAddress = async (query) => {
  try {
    const response = await apiClient.get('web/address/', { params: { query } });
...
export const getElectricityPlansByCity = async (city, pricingIds = null) => {
  try {
    ...
    const response = await apiClient.get('pricing/electricity/', { params });
...
export const getBroadbandPlansByCity = async (city, pricingIds = null) => {
  try {
    ...
    const response = await apiClient.get('pricing/broadband/', { params });
    return response.data?.plans || [];
  } catch (error) {
    return [];
  }
...
export const getPlanByPricingId = async (pricingId) => {
  try {
    const response = await apiClient.get(`pricing/plan/${pricingId}/`);
    return response.data;
  } catch (error) {
    return null;
  }
...
export const getPlansByPricingIds = async (pricingIds) => {
  try {
    ...
    const response = await apiClient.get('pricing/plans/bulk/', { params });
...
export const getMobilePlansByCity = async (pricingIds = null) => {
  try {
    ...
    const response = await apiClient.get('web/pricing/mobile/', { params });
    return response.data?.plans || [];
  } catch (error) {
