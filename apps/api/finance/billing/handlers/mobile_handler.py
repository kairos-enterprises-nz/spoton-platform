...
def calculate_bill(contract_id: str, period_start: datetime, period_end: datetime) -> Tuple[Bill, list[BillLineItem]]:
    """
    Stub mobile billing calculation (monthly, prepaid)
    """
    subtotal = Decimal('29.00')
    tax_amount = Decimal('4.35')
    total = subtotal + tax_amount

    bill = Bill(
        bill_number=f"MOB-{int(period_start.timestamp())}-{contract_id}",
    ...
    line = BillLineItem(
        description="Mobile monthly plan",
        service_type='mobile',
