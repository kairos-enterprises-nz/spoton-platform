...
const ContractCreateModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await staffApiService.createContract(formData);
      onSuccess?.(response);
      onClose();
    } catch (error) {
      ...
      if (error.status === 400 && error.data) {
        ...
        setErrors({ 
          submit: error.message || 'Failed to create contract. Please try again.' 
        });
      }
    } finally {
