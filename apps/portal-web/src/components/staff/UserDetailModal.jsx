...
const UserDetailModal = ({ isOpen, onClose, userId, onEdit, onDelete }) => {
  ...
  const tabs = [
    ...
    { id: 'accounts', name: 'Accounts', icon: CreditCard, count: userAccounts.length },
    { id: 'contracts', name: 'Contracts', icon: FileText, count: userContracts.length },
    { id: 'connections', name: 'Connections', icon: Network, count: userConnections.length },
    { id: 'plans', name: 'Plans', icon: Settings, count: userPlans.length },
    { id: 'activity', name: 'Activity', icon: Activity }
  ];

  const getTotalRelationships = () => {
    const total = userAccounts.length + userContracts.length + userConnections.length + userPlans.length;
  ...
  const handleViewAccount = (accountId) => {
    onClose();
    navigate(`/staff/accounts?selected=${accountId}`);
  };

  const handleViewContract = (contractId) => {
    onClose();
    navigate(`/staff/contracts?selected=${contractId}`);
  };

  const handleViewConnection = (connectionId) => {
    onClose();
    navigate(`/staff/connections?selected=${connectionId}`);
  };

  const handleViewPlan = (planId) => {
    onClose();
    navigate(`/staff/plans?selected=${planId}`);
  };
                      ...
                      <div className="space-y-3">
                        ...
                        {userData.user_number && (
                          <div className="flex items-center">
                            ...
                            <span className="text-sm text-gray-900 ml-2">{userData.user_number}</span>
                          </div>
                        )}
