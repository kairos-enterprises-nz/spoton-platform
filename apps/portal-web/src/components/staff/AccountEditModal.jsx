...
const AccountEditModal = ({ isOpen, onClose, onSuccess, account }) => {
  ...
  useEffect(() => {
    if (isOpen && account) {
      ...
      const accountData = {
        ...
        account_type: account.account_type || 'residential',
        billing_cycle: account.billing_cycle || 'monthly',
        billing_day: account.billing_day || 1,
        status: account.status || 'active',
        tenant: account.tenant?.id || account.tenant || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await staffApiService.updateAccount(account.id, formData);
      onSuccess?.(response);
      onClose();
    } catch (error) {
      ...
      if (error.status === 400 && error.data) {
        ...
        setErrors({ 
          submit: error.message || 'Failed to update account. Please try again.' 
        });
      }
    } finally {
    ...
    <>
        ...
        <div className="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-gray-200">
            <div>
              ...
              <p className="text-sm text-gray-500">Account #{account?.account_number || account?.id}</p>
            </div>
            <button
              onClick={handleClose}
      ...
      {showUnsavedWarning && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-60">
          <div className="relative top-1/2 transform -translate-y-1/2 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
            <div className="mt-3 text-center">
              <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-yellow-100">
                <AlertTriangle className="h-6 w-6 text-yellow-600" />
              </div>
              <h3 className="mt-4 text-lg font-medium text-gray-900">Unsaved Changes</h3>
