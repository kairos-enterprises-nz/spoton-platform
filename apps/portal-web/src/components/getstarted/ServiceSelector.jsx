        ...
        <AnimatePresence>
          {SERVICE_CONFIG.map((service) => (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
            >
              {/* Tick mark for selected - more visible */}
              {selectedServices[service.id] && (
                <span className="absolute top-1 right-1 sm:top-2 sm:right-2 z-20 bg-accent-green rounded-full p-1 sm:p-1.5 shadow-lg border-2 border-white/20">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-2 sm:h-3 w-2 sm:w-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="4">
