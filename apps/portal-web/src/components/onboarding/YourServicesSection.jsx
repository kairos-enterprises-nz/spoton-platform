...
const YourServicesSection = ({ 
  ...
  const selectedPlans = useMemo(() => {
    ...
    console.log('YourServicesSection: selectedPlans computation - getSelectedPlans result:', {
      mobilePlan: plans?.mobile ? {
        name: plans.mobile.name,
        pricing_id: plans.mobile.pricing_id,
        plan_id: plans.mobile.plan_id
      } : null
    });
    ...
    if (!plans || Object.values(plans).every(p => !p || typeof p === 'boolean')) {
      if (normalizedData?.initialSelection?.plans && Object.values(normalizedData.initialSelection.plans).some(isPlanObject)) {
        ...
        console.log('YourServicesSection: selectedPlans computation - using initialSelection plans:', {
          mobilePlan: plans?.mobile ? {
            name: plans.mobile.name,
            pricing_id: plans.mobile.pricing_id,
            plan_id: plans.mobile.plan_id
    ...
    console.log('YourServicesSection: selectedPlans computed:', plans);
    return plans;
  }, [normalizedData]);
  
  // State declarations
  const [isEditing, setIsEditing] = useState(() => {
    // Start in summary view if user has existing selections, otherwise start in editing mode
    const hasServices = selectedServices && (selectedServices.electricity || selectedServices.broadband || selectedServices.mobile);
    const hasPlans = selectedPlans && (selectedPlans.electricity || selectedPlans.broadband || selectedPlans.mobile);
    const hasAddress = selectedAddress || (selectedServices?.mobile && !selectedServices?.electricity && !selectedServices?.broadband);
    
    // Show summary view if user has pre-selected services, plans, and address (or mobile-only)
    const shouldShowSummary = hasServices && hasPlans && hasAddress;
    const initialEditing = !shouldShowSummary;
    
    console.log('YourServicesSection: Initial isEditing state:', initialEditing, {
      hasServices,
      hasPlans, 
      hasAddress,
      shouldShowSummary,
  ...
  const [tempPlans, setTempPlans] = useState(() => {
    console.log('YourServicesSection: Initializing tempPlans with selectedPlans:', selectedPlans);
  ...
  useEffect(() => {
    if (isEditing || isUserSelectingRef.current || isAutosavingRef.current) {
      console.log('YourServicesSection: Skipping state reset - isEditing:', isEditing, 'userSelecting:', isUserSelectingRef.current, 'autosaving:', isAutosavingRef.current);
      return;
    }
    
    // For mobile-only services, be extra cautious about resetting mobile selection
    const isMobileOnlyScenario = !selectedAddress && tempServices.mobile && !tempServices.electricity && !tempServices.broadband;
    if (isMobileOnlyScenario && selectedServices.mobile === false) {
      console.log('YourServicesSection: Preventing mobile service reset in mobile-only scenario');
      return;
    }
    
    console.log('YourServicesSection: RESETTING tempServices from', tempServices, 'to', selectedServices);
  ...
  }, [selectedAddress, selectedServices, selectedPlans, isEditing, tempServices]);
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        if (plansToValidate.length > 0) {
          ...
          // First, try bulk API for electricity and broadband plans
          const electricityBroadbandIds = [];
          if (selectedPlans.electricity?.pricing_id) electricityBroadbandIds.push(selectedPlans.electricity.pricing_id);
          if (selectedPlans.broadband?.pricing_id) electricityBroadbandIds.push(selectedPlans.broadband.pricing_id);
          
          if (electricityBroadbandIds.length > 0) {
            try {
              const response = await apiClient.get('/web/pricing/plans/bulk/', {
                params: { pricing_ids: electricityBroadbandIds.join(',') },
              ...
              // Map electricity and broadband plans from bulk API
              if (selectedPlans.electricity?.pricing_id && activePlanMap[selectedPlans.electricity.pricing_id]) {
                validatedPlansResult.electricity = { ...selectedPlans.electricity, ...activePlanMap[selectedPlans.electricity.pricing_id] };
              }
              if (selectedPlans.broadband?.pricing_id && activePlanMap[selectedPlans.broadband.pricing_id]) {
                validatedPlansResult.broadband = { ...selectedPlans.broadband, ...activePlanMap[selectedPlans.broadband.pricing_id] };
              }
            } catch (error) {
              console.warn('Failed to validate electricity/broadband plans via bulk API:', error);
              // Fallback: assume valid if they have pricing_id
              if (selectedPlans.electricity?.pricing_id) {
                validatedPlansResult.electricity = selectedPlans.electricity;
              }
              if (selectedPlans.broadband?.pricing_id) {
                validatedPlansResult.broadband = selectedPlans.broadband;
              }
            }
          }
          
          // Separately validate mobile plan using mobile-specific endpoint
          if (selectedPlans.mobile?.pricing_id) {
            try {
              console.log('YourServicesSection: Validating mobile plan with pricing_id:', selectedPlans.mobile.pricing_id);
              const mobileResponse = await apiClient.get('/api/web/pricing/mobile/', {
                signal: abortController.signal
              });
              const { plans: mobilePlans = [] } = mobileResponse.data || {};
              console.log('YourServicesSection: Available mobile plans for validation:', mobilePlans.map(p => ({name: p.name, pricing_id: p.pricing_id})));
              const mobilePlan = mobilePlans.find(plan => plan.pricing_id === selectedPlans.mobile.pricing_id);
              console.log('YourServicesSection: Found matching mobile plan:', mobilePlan ? {name: mobilePlan.name, pricing_id: mobilePlan.pricing_id} : 'NOT FOUND');
              if (mobilePlan) {
                const validatedPlan = { ...selectedPlans.mobile, ...mobilePlan };
                console.log('YourServicesSection: Validated mobile plan result:', {name: validatedPlan.name, pricing_id: validatedPlan.pricing_id});
                validatedPlansResult.mobile = validatedPlan;
              }
            } catch (error) {
              console.warn('Failed to validate mobile plan:', error);
              // Fallback: assume valid if it has pricing_id
              validatedPlansResult.mobile = selectedPlans.mobile;
            }
          }
  ...
  const handleAddressSelected = useCallback(async (address) => {
    if (!address) {
      // Only reset electricity and broadband services - mobile doesn't need an address
      setTempServices(prev => ({ ...prev, electricity: false, broadband: false }));
      setTempAddress(null);
  ...
  const handlePlanSelected = useCallback((plans) => {
    console.log('YourServicesSection: handlePlanSelected called with plans:', {
      mobilePlan: plans?.mobile ? {
        name: plans.mobile.name,
        pricing_id: plans.mobile.pricing_id,
        plan_id: plans.mobile.plan_id
    ...
    console.log('YourServicesSection: Setting tempPlans to:', {
  ...
  const handleConfirmChanges = useCallback(async (plansFromContinue = null) => {
    ...
    const hasChanged = JSON.stringify(tempAddress) !== JSON.stringify(initialDataRef.current.address) ||
                       JSON.stringify(tempServices) !== JSON.stringify(initialDataRef.current.services) ||
                       JSON.stringify(filteredPlans) !== JSON.stringify(initialDataRef.current.plans);
    ...
    if (hasChanged) {
      ...
      try {
        console.log('YourServicesSection: Calling onAutosave with dataToSave:', {
          mobilePlan: dataToSave.selectedPlans?.mobile ? {
            name: dataToSave.selectedPlans.mobile.name,
            pricing_id: dataToSave.selectedPlans.mobile.pricing_id,
            plan_id: dataToSave.selectedPlans.mobile.plan_id
          } : null
        });
        if (onAutosave) {
          await onAutosave(dataToSave);
          console.log('YourServicesSection: onAutosave completed successfully');
        }
        
        // Only set isEditing to false AFTER the save completes
        console.log('YourServicesSection: Setting isEditing to FALSE after successful save');
        setIsEditing(false);
        
        if (onNext) {
          // Auto-advance only when called from Continue button, not from editing
          const fromContinueButton = plansFromContinue !== null;
          onNext(dataToSave, fromContinueButton);
        }
      } catch (error) {
      ...
      // No changes detected, but still set isEditing to false
      console.log('YourServicesSection: No changes detected, setting isEditing to FALSE');
      setIsEditing(false);
      
      if (onNext) {
        // No auto-advance when no changes detected
        onNext({
          yourServices: {
            addressInfo: tempAddress,
            selectedAddress: tempAddress,
  ...
  useEffect(() => {
    return () => {
      const hasChanged = JSON.stringify(tempAddress) !== JSON.stringify(initialDataRef.current.address) ||
                         JSON.stringify(tempServices) !== JSON.stringify(initialDataRef.current.services) ||
                         JSON.stringify(tempPlans) !== JSON.stringify(initialDataRef.current.plans);

      if (hasChanged && onAutosave) {
        onAutosave({
          yourServices: {
            addressInfo: tempAddress,
            selectedAddress: tempAddress,
            selectedServices: tempServices,
            selectedPlans: tempPlans,
            lastModified: new Date().toISOString(),
            completed: true,
            hasPower: tempServices.electricity,
            hasBroadband: tempServices.broadband,
  ...
  }, [tempAddress, tempServices, tempPlans, onAutosave]);
  ...
  const handleCancelEdit = useCallback(() => {
    ...
    setTempServices(selectedServices);
    setTempPlans(selectedPlans);
    console.log('YourServicesSection: Setting isEditing to FALSE in handleCancelEdit');
    setIsEditing(false);
  }, [selectedAddress, selectedServices, selectedPlans]);
  ...
  const currentPlans = useMemo(() => {
    if (isEditing) {
      console.log('YourServicesSection: currentPlans = tempPlans (editing mode)');
      return tempPlans;
    }
    if (validatedPlans && selectedPlans) {
      ...
      if (currentIds === lastValidatedIds) {
        console.log('YourServicesSection: currentPlans = validatedPlans (validation match)', {
          validatedMobile: validatedPlans.mobile ? {name: validatedPlans.mobile.name, pricing_id: validatedPlans.mobile.pricing_id} : null
        });
        return validatedPlans;
      }
    }
    console.log('YourServicesSection: currentPlans = selectedPlans (default)', {
      selectedMobile: selectedPlans?.mobile ? {name: selectedPlans.mobile.name, pricing_id: selectedPlans.mobile.pricing_id} : null
    });
    return selectedPlans;
  ...
  useEffect(() => {
    console.log('YourServicesSection: tempServices changed:', tempServices);
    let isMounted = true;
    if (tempServices?.mobile) {
      console.log('YourServicesSection: Mobile service selected, loading plans...');
      setIsLoadingMobilePlans(true);
      setMobilePlansError(null);
      getMobilePlansByCity()
        .then((plans) => {
          console.log('YourServicesSection: Mobile plans loaded:', plans);
          if (isMounted) {
            ...
            console.log('YourServicesSection: Transformed mobile plans:', transformedPlans);
  ...
  const showPlans = (
    // Show plans if mobile is selected (mobile doesn't need address)
    tempServices?.mobile ||
    // OR if electricity/broadband is selected AND address is provided
    ...
    return (
      ...
      >
        <div className={`relative w-full p-6 rounded-2xl transition-all duration-300 bg-gradient-to-br ${colorClass}`}>
          {/* Edit button in top-right corner when not editing */}
          {!isEditing && (
            <button
              onClick={() => setIsEditing(true)}
              className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 flex items-center justify-center transition-all duration-200 group"
              title="Edit plan"
            >
              <PencilIcon className="w-4 h-4 text-white/70 group-hover:text-white transition-colors" />
            </button>
          )}
  ...
  return (
    <div className="max-w-3xl mx-auto bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#334155] rounded-2xl border border-white/10 p-8 shadow-lg backdrop-blur-md">
      ...
      ) : selectedAddress || (tempServices?.mobile && !tempServices?.electricity && !tempServices?.broadband) ? (
        <div className="space-y-6">
