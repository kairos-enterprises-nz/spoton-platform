...
export default function PowerPage() {
  ...
  const hasPowerService = userServices?.includes('electricity');
  const powerPlan = portalData?.services?.selected_plans?.electricity;
  ...
  if (!hasPowerService) {
    return (
      <ComingSoonCard
        title="Power"
        subtitle="Hassle-free switching, usage insights, and a fair tariff model you can actually understand"
        description="Service Coming Soon"
        theme="electricity"
        actionText="Get Notified"
        serviceType="power"
        onAction={() => {
          console.log('User wants power service notifications');
        }}
      />
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <PageHeader
        ...
        subtitle="Hassle-free switching, usage insights, and a fair tariff model you can actually understand"
        icon={Zap}
        statusText={powerData.status === 'active' ? 'Connected' : 
                   powerData.status === 'switching' ? 'Switching' :
                   powerData.status === 'pending' ? 'Pending Switch' :
                   powerData.statusText || 'Coming Soon'}
        statusColor={powerData.status === 'active' ? 'accent-green' : 'accent-orange'}
        theme="power"
        rightContent={
          <>
            {getTimeOfDayIcon()}
            <div className="text-left">
              ...
              <div className="text-sm font-semibold">
                {powerData.status === 'active' ? 'Connected' : 
                 powerData.status === 'switching' ? 'Switching' :
                 powerData.status === 'pending' ? 'Pending Switch' :
                 powerData.statusText || 'Coming Soon'}
              </div>
