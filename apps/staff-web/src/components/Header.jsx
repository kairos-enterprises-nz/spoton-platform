...
export default function Header() {
  ...
  return (
    <header className="bg-white">
      <nav className="mx-auto flex max-w-7xl h-14 md:h-16 items-center justify-between gap-x-6 px-4 md:px-6 lg:px-8" aria-label="Global">
        {/* Logo */}
        <div className="flex lg:flex-1">
          <button onClick={() => handleStart('/')} className="-m-1.5 p-1.5">
            <span className="sr-only">Spot On</span>
            <img src={Logo} alt="Spot On Logo" className="h-8 md:h-9 lg:h-10 w-auto cursor-pointer" />
          </button>
        </div>
        ...
        <div className="hidden lg:flex lg:gap-x-18">
          {navigation.map((item) => (
            <button
              ...
              className="rounded-full px-3 py-1.5 border-transparent font-semibold text-sm md:text-base text-gray-900 border-2 hover:bg-accent-purple hover:text-white hover:font-bold cursor-pointer"
            >
              {item.name}
            </button>
          ))}
        ...
        <Dialog.Panel className="absolute top-0 right-0 z-50 w-full h-full max-h-screen bg-white px-6 py-6 overflow-y-auto sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          ...
          <div className="flex flex-col items-center gap-y-4">
            <button onClick={() => handleStart('/')} className="p-1.5">
              <img src={Logo} alt="Spot On Logo" className="h-8 w-auto cursor-pointer" />
            </button>
          </div>
