...
const CURRENT_NZ_DATE = new Date("2025-05-03T10:42:01+12:00");
...
export default function AboutYouSection({ initialData = {}, onNext, onAutosave, user }) {
  ...
  // Populate form from initialData prop when it changes (e.g., on mount with fetched data)
  useEffect(() => {
    if (initialData && Object.keys(initialData).length > 0) {
      setForm((prev) => ({
        ...prev,
        legalFirstName: initialData.legalFirstName || userData.first_name || prev.legalFirstName || '',
        legalLastName: initialData.legalLastName || userData.last_name || prev.legalLastName || '',
        preferredName: initialData.preferredName || userData.preferred_name || prev.preferredName || '',
        middleName: initialData.middleName || userData.middle_name || prev.middleName || '',
        dob: initialData.dob || userData.dob || prev.dob || '',
        consentCreditCheck: initialData.consentCreditCheck !== undefined ? initialData.consentCreditCheck : prev.consentCreditCheck,
        idType: initialData.idType || prev.idType || '',
        driverLicenceNumber: initialData.driverLicenceNumber || prev.driverLicenceNumber || '',
        driverLicenceVersion: initialData.driverLicenceVersion || prev.driverLicenceVersion || '',
        passportNumber: initialData.passportNumber || prev.passportNumber || '',
        email: userData.email || prev.email || '',
        mobile: userData.mobile || userData.phone_number || prev.mobile || '',
      }));
    } else if (userData && Object.keys(userData).length > 0) {
      setForm((prev) => ({
        ...prev,
        legalFirstName: prev.legalFirstName || userData.first_name || '',
        legalLastName: prev.legalLastName || userData.last_name || '',
        preferredName: prev.preferredName || userData.preferred_name || '',
        middleName: prev.middleName || userData.middle_name || '',
        dob: prev.dob || userData.dob || '',
        email: userData.email || prev.email || '',
        mobile: userData.mobile || userData.phone_number || prev.mobile || '',
      }));
    }
  ...
  // If user is provided as a prop, use it to prefill form fields on mount
  useEffect(() => {
    if (user) {
      setForm((prev) => ({
        ...prev,
        legalFirstName: user.first_name || prev.legalFirstName,
        legalLastName: user.last_name || prev.legalLastName,
        // Don't overwrite preferredName, middleName, dob if already set
      }));
    }
  }, [user]);
  ...
  const validate = useCallback((formData) => {
    const newErrors = {};
    const { dob, consentCreditCheck, idType, driverLicenceNumber, driverLicenceVersion, passportNumber } = formData;
  ...
  const handleSubmit = useCallback((e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      const enhancedPayload = {
        ...
        mobile: payload.mobile || userData.mobile || ''
      };
