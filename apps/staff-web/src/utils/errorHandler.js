...
class ErrorHandler {
  constructor() {
    // Normalize to only two environments
    this.environment = typeof __ENVIRONMENT__ !== 'undefined' ? __ENVIRONMENT__ : 'uat';
  ...
  handleError(error, context = 'Unknown', metadata = {}) {
    ...
    if (isUAT) {
      ...
      // Non-standard: treat as UAT-style detailed logging
      logger.error('UAT Error:', errorInfo);
    }
  ...
  handleApiError(error, endpoint = 'unknown') {
    ...
    if (isUAT) {
      ...
      logger.error('API Error (UAT-like):', apiErrorInfo);
    }
