...
class StaffApiService {
  constructor() {
    // Staff API namespace - using clean /staff/v1 path (no extra /api segment)
    // Combined with axios baseURL (api subdomain), results in https://<env>.api.spoton.co.nz/staff/v1
    this.baseURL = '/staff/v1';
  }
  ...
  getCurrentTenantFilter() {
    try {
      ...
      if (user.is_superuser) {
        // Super admin: use selected tenant, null means "All Tenants"
        return selected || null;
      }
      
      // For regular users, prefer their tenant_id; if absent (e.g., staff tokens without tenant claim),
      // fall back to the selected tenant (single-tenant setups)
      const tenantFilter = user.tenant_id || selected || null;
      
      // Debug logging for troubleshooting
      if (!tenantFilter) {
        console.warn('⚠️ No tenant filter found', { 
  ...
  addTenantFilter(params = {}) {
    ...
    if (tenantFilter) {
      ...
      // For superusers without tenant selection, request all data
      // For regular users, this should not happen (they should have tenant_id)
      params.all = 1; // Explicitly request all tenants when no tenant selected
      console.log('🔍 No tenant filter applied - requesting all tenant data');
    }
    return params;
  }
  ...
  async getUsersStats() {
    try {
      ...
      this.handleError(error, 'get users stats');
    }
  }

  async getUsers(params = {}) {
    try {
      const filteredParams = this.addTenantFilter({ ...params, ts: Date.now() });
  ...
  async getAccount(accountId) {
    try {
      ...
      const params = this.addTenantFilter({});
      const response = await apiClient.get(`${this.baseURL}/accounts/${accountId}/`, { params });
      return this.handleResponse(response);
    } catch (error) {
  ...
  async getContract(contractId) {
    try {
      ...
      const params = this.addTenantFilter({});
      const response = await apiClient.get(`${this.baseURL}/contracts/${contractId}/`, { params });
      return this.handleResponse(response);
    } catch (error) {
  ...
  async getPlans(params = {}) {
    try {
      // CRITICAL FIX: Add tenant filtering to plans API call
      const filteredParams = this.addTenantFilter(params);
      const response = await apiClient.get(`${this.baseURL}/plans/`, { params: filteredParams });
      return this.handleResponse(response);
    } catch (error) {
  ...
  async updateConnectionStatus(connectionId, newStatus) {
    try {
      this.validateRequired({ connectionId, newStatus }, ['connectionId', 'newStatus'], 'update connection status');
      const response = await apiClient.post(`${this.baseURL}/connections/${connectionId}/update_status/`, {
        status: newStatus
      });
      return this.handleResponse(response);
    } catch (error) {
      this.handleError(error, 'update connection status');
    }
  }

  async assignConnectionToContract(connectionId, contractId) {
    try {
      this.validateRequired({ connectionId, contractId }, ['connectionId', 'contractId'], 'assign connection to contract');
      const response = await apiClient.post(`${this.baseURL}/connections/${connectionId}/assign_to_contract/`, {
        contract_id: contractId
      });
      return this.handleResponse(response);
    } catch (error) {
      this.handleError(error, 'assign connection to contract');
    }
  }

  async unassignConnectionFromContract(connectionId) {
    try {
      this.validateRequired({ connectionId }, ['connectionId'], 'unassign connection from contract');
      const response = await apiClient.post(`${this.baseURL}/connections/${connectionId}/unassign_from_contract/`);
      return this.handleResponse(response);
    } catch (error) {
      this.handleError(error, 'unassign connection from contract');
    }
  }
  ...
  async exportTenants(format = 'csv', params = {}) {
    try {
      ...
      // Fallback for environments routing export under /api prefix
      if (error?.response?.status === 404) {
        try {
          const fallback = await apiClient.get(`/api${this.baseURL}/tenants/export/`, {
            params: { format, ...params },
            responseType: 'blob'
          });
          return fallback.data;
        } catch (e) {
          this.handleError(e, 'export tenants (fallback)');
        }
      }
  ...
  async exportUsers(format = 'csv', params = {}) {
    try {
      ...
      if (error?.response?.status === 404) {
        try {
          const fallback = await apiClient.get(`/api${this.baseURL}/users/export/`, {
            params: { format, ...params },
            responseType: 'blob'
          });
          return fallback.data;
        } catch (e) {
          this.handleError(e, 'export users (fallback)');
        }
      }
  ...
  async exportContracts(format = 'csv', params = {}) {
    try {
      ...
      if (error?.response?.status === 404) {
        try {
          const fallback = await apiClient.get(`/api${this.baseURL}/contracts/export/`, {
            params: { format, ...params },
            responseType: 'blob'
          });
          return fallback.data;
        } catch (e) {
          this.handleError(e, 'export contracts (fallback)');
        }
      }
  ...
  async exportAccounts(format = 'csv', params = {}) {
    try {
      const response = await apiClient.get(`${this.baseURL}/accounts/export/`, {
        params: { format, ...params },
        responseType: 'blob'
      });
      return response.data;
      ...
      if (error?.response?.status === 404) {
        try {
          const fallback = await apiClient.get(`/api${this.baseURL}/accounts/export/`, {
            params: { format, ...params },
            responseType: 'blob'
          });
          return fallback.data;
        } catch (e) {
          this.handleError(e, 'export accounts (fallback)');
        }
      }
      this.handleError(error, 'export accounts');
    }
  }

  async exportConnections(format = 'csv', params = {}) {
    try {
      const response = await apiClient.get(`${this.baseURL}/connections/export/`, {
        params: { format, ...params },
        responseType: 'blob'
      });
      return response.data;
      ...
      if (error?.response?.status === 404) {
        try {
          const fallback = await apiClient.get(`/api${this.baseURL}/connections/export/`, {
            params: { format, ...params },
            responseType: 'blob'
          });
          return fallback.data;
        } catch (e) {
          this.handleError(e, 'export connections (fallback)');
        }
      }
      this.handleError(error, 'export connections');
    }
  }

  async exportPlans(format = 'csv', params = {}) {
    try {
      const response = await apiClient.get(`${this.baseURL}/plans/export/`, {
        params: { format, ...params },
        responseType: 'blob'
      });
      return response.data;
      ...
      if (error?.response?.status === 404) {
        try {
          const fallback = await apiClient.get(`/api${this.baseURL}/plans/export/`, {
            params: { format, ...params },
            responseType: 'blob'
          });
          return fallback.data;
        } catch (e) {
          this.handleError(e, 'export plans (fallback)');
        }
      }
      this.handleError(error, 'export plans');
    }
  }
